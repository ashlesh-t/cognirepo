...
def test_cli_help():
    # Use subprocess to exercise cli/main.py entry point
    res = subprocess.run([sys.executable, "-m", "cli.main", "--help"], capture_output=True, text=True)
    assert res.returncode == 0
    assert "CogniRepo" in res.stdout

def test_cli_doctor_cmd(isolated_cognirepo):
    # doctor cmd exercises many imports and basic logic. 
    # It might return exit code 2 if index is missing, but we just want to exercise the code.
    res = subprocess.run([sys.executable, "-m", "cli.main", "doctor"], capture_output=True, text=True)
    assert "CogniRepo doctor" in res.stdout
