...
def test_count_tokens_nonempty():
    from interface.tools.benchmark import _count_tokens
...
def test_print_report_with_metrics(capsys):
    from interface.tools.benchmark import print_report
...
def test_measure_latency_no_golden():
    ...
    with patch("interface.tools.context_pack.context_pack", return_value=mock_result):
...
def test_measure_symbol_lookup_mocked():
    from interface.tools.benchmark import measure_symbol_lookup
...
def test_measure_memory_recall_mocked():
    from interface.tools.benchmark import measure_memory_recall
    with patch("interface.tools.store_memory.store_memory", return_value={"status": "stored"}):
        with patch("interface.tools.retrieve_memory.retrieve_memory", return_value=[{"text": "test memory", "score": 0.9}]):
...
def test_measure_cache_speedup_mocked():
    from interface.tools.benchmark import measure_cache_speedup
    mock_result = {"query": "test", "sections": [], "token_count": 10, "status": "ok"}
    with patch("interface.tools.context_pack.context_pack", return_value=mock_result):
...
def test_measure_context_relevance_mocked():
    from interface.tools.benchmark import measure_context_relevance
    mock_result = {"query": "test", "sections": [{"score": 0.8}], "token_count": 100, "status": "ok"}
    with patch("interface.tools.context_pack.context_pack", return_value=mock_result):
