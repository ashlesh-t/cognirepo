...
def test_days_old_with_timestamp():
    from ops.cron.prune_memory import _days_old
...
def test_recency_decay_zero():
    from ops.cron.prune_memory import _recency_decay
...
def test_final_score_high_importance_recent():
    from ops.cron.prune_memory import _final_score
    ts = datetime.now(tz=timezone.utc).isoformat()
...
def test_prune_no_metadata_file(monkeypatch, tmp_path):
    import ops.cron.prune_memory as pm
    monkeypatch.setenv("COGNIREPO_DIR", str(tmp_path / ".cognirepo"))
    with patch("ops.cron.prune_memory._semantic_meta", return_value=str(tmp_path / "missing.json")):
        with patch("ops.cron.prune_memory._check_memory_pressure", return_value=True):
...
def test_prune_circuit_open(tmp_path):
    import ops.cron.prune_memory as pm
    with patch("ops.cron.prune_memory._check_memory_pressure", return_value=False):
        result = pm.prune(dry_run=True)
...
def test_prune_dry_run_with_entries(tmp_path):
    ...
    with patch("ops.cron.prune_memory._semantic_meta", return_value=meta_path):
        with patch("ops.cron.prune_memory._check_memory_pressure", return_value=True):
            with patch("ops.cron.prune_memory._prune_graph", return_value={"orphans_removed": 0, "cold_nodes_removed": 0}):
...
def test_prune_graph_no_pkl(tmp_path):
    import ops.cron.prune_memory as pm
    with patch("ops.cron.prune_memory._graph_pkl", return_value=str(tmp_path / "missing.pkl")):
...
def test_prune_graph_dry_run(tmp_path):
    ...
    with open(pkl_path, "w") as f:
        f.write("fake")
    with patch("ops.cron.prune_memory._graph_pkl", return_value=pkl_path):
